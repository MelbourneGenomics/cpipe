...
vcf_annotate = {
    ...
    def infoField
    if(USE_REFSEQ) {
        println "Annotating using RefSeq"
        filterType='vep'
        refSeqFlag = '--refseq '
        infoField="ANN"
    }
    else {
        println "Annotating using Ensemble"
        filterType='vep.ens'
        refSeqFlag = ''
        infoField="CSQ"
    }
    ...
    exec """
        ...
        then
          cp $input.vcf $output.vcf;
            ...
            PERL5LIB="$PERL5LIB:$TOOLS/perl5:$TOOLS/perl5/lib/perl5:$VEP" $PERL $VEP/filter_vep.pl 
...
vcf_vcfanno = {
    ...
    if (VCFANNO_CONFIG != "") {
        ...
        exec """
            ...
            sed 's,\$ANNOTATIONS,$ANNOTATIONS,g' < $VCFANNO_CONFIG > $safe_tmp_dir/vcfanno.config;
...
transcript_filter = {
 ...
 create_seqr_project = {
     output.dir = 'seqr'
     produce('project.yaml') {
         exec """

             echo "JAVA_HOME=\$JAVA_HOME"

             $GROOVY -cp $GROOVY_NGS/groovy-ngs-utils.jar $SCRIPTS/CreateSeqrProject.groovy -sample_info ../samples.txt
         """
     }
 }
