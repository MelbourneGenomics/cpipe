...
def analyze_log(log, result):
    ...
    for line in log:
        ...
        if ": enter " in line:
            ...
            when = datetime.datetime(int(when[:2]), int(when[2:4]), int(when[4:6]), int(when[7:9]), int(when[9:11]),
                                     int(when[11:13]))
            ...
            if len(in_progress[identifier]) > 0:
                ...
                new_time = datetime.datetime(int(when[:2]), int(when[2:4]), int(when[4:6]), int(when[7:9]),
                                             int(when[9:11]), int(when[11:13]))
    ...
    for key in sorted(final.keys()):
        result.write('{0}\t{1}\t{2}\t{3:.0f}\t{4}\t{5:.0f}\n'.format(key, min(final[key]), max(final[key]),
                                                                     sum(final[key]) / len(final[key]), sum(final[key]),
                                                                     sum(final[key]) * 100 / total))
